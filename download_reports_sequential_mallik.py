...
specific_cilents = [640, 655, 822, 3650, 3657, 3696, 3698, 3716, 3724]
specific_cilents = [3671]

for specific_client in specific_cilents:
    credentials = sql.execute_query(CRED_Q.format(client_id=specific_client))

    client_id, client_name, username, password = credentials[0]
    print(f'Executing for client ID: {client_id}')
    ...
    # print("Completed MED_1")

    # # Extract, Transform, Load PER_2
    # exct_rep.per_02(per_2_cfg['report_name'], per_2_cfg['from_date'], per_2_cfg['to_date'])
    # task_q.add_task(trns_csv.per_02, os.path.join(DWLD_DIR, per_2_cfg['file_name']), os.path.join(DWLD_DIR, per_2_cfg['processed_file']))
    # task_q.add_task(load_csv.load_report, os.path.join(DWLD_DIR, per_2_cfg['processed_file']), per_2_cfg['base_table'], per_2_cfg['staging_table'])
    # task_q.add_task(file_folder.move_file, os.path.join(DWLD_DIR, per_2_cfg['processed_file']), CLIENT_TODAY_DIR)
    # print("Completed PER_2")
    ...
    # task_q.add_task(trns_csv.combine_csv_files, DWLD_DIR, os.path.join(DWLD_DIR, rev_16_cfg['file_name']), rev_16_cfg['report_name'])
